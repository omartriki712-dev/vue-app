  ...
  <div class="flex-1 flex items-center justify-center">
    <div class="w-full max-w-[448px] px-4">
      ...
      <div class="bg-white rounded-2xl border border-[#e4e0eb] shadow-xl overflow-hidden transition-all" :class="{ 'shake': error }">
        <form class="p-8" @submit="handleLogin">
          <div class="space-y-6">
...
const handleLogin = (e) => {
  ...
  loading.value = true;
  login({usernameOrEmail: email.value, password: password.value})
    .then(res => {
      if (!res) {
        router.push('/dashboard/student/semesters');
      } else {
